...
gulp.task('sass-for-build', () =>
	sass('sass/main.scss', {sourcemap: false})
	.on('error', sass.logError)
	// .pipe(sourcemaps.write())
	.pipe(autoprefixer(['last 2 versions']))
	.pipe(gulp.dest('styles'))
...
gulp.task('serve', ['sass', 'fileinclude'], () => {
	...
	gulp.watch('devscripts/*.js', ['babel']).on('change', browserSync.reload);
...
gulp.task('build', ['sass-for-build', 'cleanDist', 'minifycss'], () => {
